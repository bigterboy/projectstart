...
export default class Screen extends BaseScreen {
  ...
  verifyOTPAPI = async () => {
    try {
      ...
      const request = {
        ...
        otpType: Util.Constant.OTP_TYPE.ESIGN,
      ...
      if (result != null) {
        if (result.code === 200) {
          setTimeout(() => {
            this.props.navigation.navigate("LoanEsignComplete");
  ...
  render() {
    ...
    return (
      <HDEsignHeader subTitle={2} step={3}>
        <DismissKeyboardView>
          <KeyboardAvoiding style={styles.avoidKeyboardContainer}>
            <HDText style={styles.title}>
              <HDText>
                {Context.getString("loan_tab_esign_enter_otp_guild")}
              </HDText>
              <HDText style={styles.security_phone_text}>
                {securityPhone}
              </HDText>
              <HDText>
