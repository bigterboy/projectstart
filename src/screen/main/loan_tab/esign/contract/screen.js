...
import {
  BaseScreen, HDButton, HDPdf,
  HDText, HDEsignHeader, HDAnimatedLoading
} from "component";
...
export default class Screen extends BaseScreen {
  constructor(props) {
    super(props);
    this.state = {
      checked: false, //Checked User Agreement
      base64pdf: "",
      loadComplete: false,
      isLoading: true
    };
    ...
    if (contractBase64) {
      ...
      try {
        var result = await Network.esignCreateFileB64(
          ...
          Util.Constant.CONTRACT_FILE_TYPE.ESIGN
        );

        if (result != null) {
          if (result.code === 200) {
            console.log(
              "API-CREATE-FILE-CONTRACT-B64-COMPLETE" + JSON.stringify(result)
            );
            if (result.payload) {
              this.props.saveContractBase64(result.payload);
              this.setState({
  ...
  checkLogAPI = async checkLogType => {
    const { objEsign } = this.props;
    try {
      ...
      if (result != null) {
        if (result.code === 200) {
          console.log(
            "API-ESIGN-CHECK-LOG-COMPLETE" + JSON.stringify(result.payload)
          );
        } else {
  ...
  onPressCheckbox = async () => {
    this.setState(
      ...
      async () => {
        ...
        if (checked)
          await this.checkLogAPI(Util.Constant.CHECK_LOG_ACTION.READ_AGREE);
  ...
  onPressConfirm = () => {
    this.checkLogAPI(Util.Constant.CHECK_LOG_ACTION.CONFIRM_SIGN_CONTRACT);
    this.props.updateEsignStep(3);

    this.props.navigation.navigate("LoanESignSign");
  };
  ...
  renderPolicy = () => {
    ...
    return (
      <View
        style={{ width: "100%", height: "100%", paddingBottom: bottomHeight }}
      >
        <HDPdf source={source} onLoadComplete={this.loadFileComplete}></HDPdf>
      </View>
    );
  };
        ...
        <View style={styles.bottom_container}>
          ...
          >
            <CheckBox
              ...
              checkedIcon={
                <Image
                  source={Context.getImage("iconChecked")}
                  style={styles.checkbox}
                />
              }
              uncheckedIcon={
                <Image
                  source={Context.getImage("iconUnChecked")}
                  style={styles.checkbox}
                />
              }
              checked={this.state.checked}
            ...
            <View style={{ flex: 1 }}>
              <HDText>
                <HDText style={styles.agree_text}>
                  {Context.getString("loan_tab_esign_contract_document_agree")}
                </HDText>
              </HDText>
            </View>
          </TouchableOpacity>
  ...
  render() {
    return (
      <HDEsignHeader navigation={this.props.navigation} subTitle={1} step={2}>
        {this.renderLoading()}
