...
export default class Screen extends BaseScreen {
  ...
  validateSignInAPI = async (username, password, isEncrypt) => {
    ...
    try {
      ...
      if (result != null) {
        if (result.code === 200) {
          console.log(
            "API-VALIDATE-SIGNIN-COMPLETE: " + JSON.stringify(result.code)
          );
          this.props.navigation.navigate("LoanESignEnterOtp", {
            navData: {
          ...
          console.log(
            "ERROR-API-VALIDATE-SIGNIN: " + JSON.stringify(result.code)
          );
          console.log(
            "ERROR-API-VALIDATE-SIGNIN: " +
      ...
      if (currName === bioName) {
        Util.FaceID.auth(async error => {
          if (error.isError) {
            ...
            this.validateSignInAPI(username, encrypted, true);
          }
        });
      } else {
        showError();
  ...
  render() {
    const { biometricType } = this.props;
    return (
      <HDEsignHeader navigation={this.props.navigation} subTitle={2} step={3}>
        <DismissKeyboardView style={styles.container}>
          <HDText style={styles.status}>
            {Context.getString("loan_tab_esign_sign_status")}
          </HDText>
